# Decision: Protect the Gene Keys Prompt Lab with Pages Functions
...
Add a narrow Cloudflare Pages Functions boundary for the Prompt Lab while leaving all other frontend
routes static. Store the two 64-chapter sets in a private Workers KV namespace and expose neither the
namespace nor its values to the browser. Store the shared password and an independent HMAC signing
secret as encrypted Pages secrets. Successful login creates a 12-hour HttpOnly, Secure,
SameSite=Strict cookie; POST endpoints also enforce same-origin requests.
...
4. Pages Functions, private KV, Workers AI, and device-local history — accepted as the smallest
   deployable boundary that keeps evidence and credentials server-side.
...
- The source upload is an explicit, separately authenticated operational command. It does not adopt
  the newer *Gene Keys* chunks into the tracked commentary provenance graph.
...
- [`../../../functions/api/gene-keys-lab`](../../../functions/api/gene-keys-lab)
