...
import { getShichenIndex } from '@/domain/astrology/shichen'
...
export const getNextEarthlyBranchBoundaryUtc = (at: Date, timezone: string) => {
  ...
  const currentBranch = getShichenIndex(getZonedCivilTime(at, timezone).hour)
  ...
  for (let offset = 0; offset <= MAX_SCAN_MINUTES; offset += 1) {
    ...
    if (civil.minute === 0 && getShichenIndex(civil.hour) !== currentBranch) {
