...
def test_bounded_post_routes_are_explicitly_private(path: str) -> None:
    policy = endpoint_policy("POST", path)

    assert policy.endpoint_class is EndpointClass.PRIVATE_NO_STORE
    assert policy.rate_class is RatePolicyClass.AUTHENTICATED_READ
    assert policy.cache_control == "private, no-store"
